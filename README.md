...
* 수열(nth) 선택자
    ...
    * `형제선택자:nth-child(odd)` : 홀수번째 형제
    * `형제선택자:nth-child(even)`: 짝수번째 형제
# css 선택자 우선순위
* `<style>` > `#` > `.` > `tag`
* 1. `<tag style="">` 우선순위 높음
* 2. `#id {}`
...
* 자식이 2개 이상일 때 부모에 속성을 적용하는 것으로 자식에게 공통 값을 적용할 수 있다.
    ...
    * 꾸미려는 css 속성이 글자에 관련된 속성(글꼴, 글자크기, 글자색상 등) 본인에게 직접 적용하는 것을 권장함. (우선순위가 꼬여서 안됨)
## 글꼴 사용시 주의 사항
...
* `display:;` : 요소의 형태유형 변경속성
    ...
    * inline-block값은 모두 적용 가능
        ...
        * 크기(w100%(가까운부모의100%), h(hug:내용물만큼)와 여백값을 인식
        * 옆으로 나열(hug)시킴, 넘치면 자동 줄바꿈됨.
* `margin` : 바깥쪽(형제 사이) 여백(피그마 기준 **간격**)
    * `margin 10px 20px;` : 상하좌우
...
* `table` -> `tfoot` -> `tr` -> `td`
## 수평/수직 열 합치기
* 합치기 속성은 무조건 `th` 또는 `td` 태그에 입력 가능! 
* 수평 열 합치기 
...
= `li {width:calc((100% - (10px * 3)px) / 4);}`
    * 4개의 li에 각 10px씩 사이 여백을 주기 위해 전체 부모 100%의 너비 중 10*3 총 30px를 빼고 나머지값을 4로 나누기
* `a {display:block; height:calc(100% - 50px);}`
    * a의 크기를 인식하게 만들고 50px를 뺀 나머지 부모 크기 주기
## 수평/수직 정렬 레이아웃 속성 Flex
### 정렬 순서
1. 정렬하고자 하는 **두개 이상의 형제** 대상 체크
    * `ul > li*5 > a` -> 형제 `li`
...
### 메인축 정렬 속성
* `justify-contents` :
    * `flex-starts` : 메인축이 수직이면 위쪽, 수평이면 왼쪽
    * `flex-end` : 메인축이 수직이면 아래, 수평이면 오른쪽
    * `space-between` : 메인축이 수직이면 위-아래 양쪽 끝, 수평이면 왼쪽-오른쪽 양쪽 끝
    * `space-around` : 메인축이 수직이면 위-아래 양쪽 여백 주고 균등배치, 수평이면 왼쪽-오른쪽에 양쪽 여백 주고 균등배치
    * `center` : 메인축이 수직이면 수직중앙, 수평이면 수평중앙
### 교차축 정렬 속성
* `aline-item` : 교차축이 1줄일때
    * `flex-starts`, `flex-end`, `center` 위 메인축과 뜻 동일
* `aline-contents` : 교차축이 2줄 이상일때
    * `flex-starts`, `flex-end`, `center` , `space-between`, `space-around` 위 메인축과 뜻 동일, 값 동일
## 스크롤바 디자인 css
...
* `:hover` 선택자로 인해 기존 속성에서 다른 속성으로 변경되는 css에 애니메이션을 주고 싶을 때
    ...
    * 2초 기다리고 1초동안 일정 속도로 color 속성에 애니적용
## animation + @keyframes
### 적용순서
1. `css 애니메이션 이름 {적용분기}`
...
* `keyframes + transition`은 페이지 로딩 시 등장 애니메이션, 스크롤 내릴 시 나타나는 컨텐츠의 등장 애니메이션 등에 사용함. (사용자 hover 동작과 대부분 관계없음)
### 명령 해석
* `@keyframes a {0% {opacity:0;} 100% {opacity:1;}}`
    * 애니메이션이름 a 생성 후 안보이다가 보이는 애니메이션
* 선택자 {animation: a 1s ease 0.5s alternate 3;}
    * 특정 선택자에 0.5초 기다리고 1초 동안 a 애니메이션을 ease 가속도로 정방향->역방향 순서로 3번 재생하기(무한 재생은 `infinite`)
## transform
